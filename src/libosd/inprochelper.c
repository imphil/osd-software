...
#ifdef DEBUG
# define DEBUG_INPROCHELPER
#endif

void inprochelper_send_data(zsock_t *socket, const char* name, const void* data,
                            size_t size)
{
    int zmq_rv;
...
{
    inprochelper_send_data(socket, name, &value, sizeof(int));
}
...
/**
 * Handler: Message from main thread received in communication thread
 */
static int thread_inproc_rcv(zloop_t *loop, zsock_t *reader,
                             void *thread_ctx_void)
{
    struct inprochelper_thread_ctx *thread_ctx = thread_ctx_void;
    assert(thread_ctx);
 ...
 *
 * @param thread_ctx_void the thread context object. The thread takes over
...
static void* thread_main(void *thread_ctx_void)
{
    ...
    if (thread_ctx->init_fn) {
        ...
        if (OSD_FAILED(osd_rv)) {
            inprochelper_send_status(thread_ctx->inproc_socket,
                                     "I-THREADINIT-DONE", osd_rv);
    ...
    inprochelper_send_status(thread_ctx->inproc_socket, "I-THREADINIT-DONE",
                             OSD_OK);
                            ...
                            void* thread_ctx_usr)
{
    ...
    struct inprochelper_ctx *c = calloc(1, sizeof(struct inprochelper_ctx));
    assert(c);
    ...
    int retval;
    inprochelper_wait_for_status(c->inproc_socket, "I-THREADINIT-DONE", &retval);
...
void inprochelper_free(struct inprochelper_ctx **ctx_p)
{
    ...
    assert(ctx_p);
    struct inprochelper_ctx *ctx = *ctx_p;
    if (!ctx) {
        return;
    }

    // shut down thread
    inprochelper_send_status(ctx->inproc_socket, "I-SHUTDOWN", 0);

    // wait for shutdown to happen
    int retvalue;
    osd_rv = inprochelper_wait_for_status(ctx->inproc_socket, "I-SHUTDOWN-DONE",
                                          &retvalue);
    if (OSD_FAILED(osd_rv)) {
